...
module Admins
  class MerchantMethodsController < Staff::BaseController
    ...
    def create
      ...
      redirect_to settings_merchant_path(@merchant)
    end

    def destroy
      if @merchant.merchant_methods.destroy(params[:id])
        redirect_to settings_merchant_path(@merchant), notice: 'Метод успешно удалён', status: :see_other
      else
        redirect_to settings_merchant_path(@merchant), alert: 'Ошибка удаления метода', status: :see_other
      end
    end
