...
module RateSnapshots
  class GetBybitP2pRatesJob
    include Sidekiq::Job
    sidekiq_options queue: 'low', tags: ['bybit_p2p_rates']
    ...
    def perform(params)
      ...
      Bybit::RateSnapshot.new(params.merge({ action: 'buy', payment_system:,
                                               fiat_amount: payment_system.trans_amount_deposit,
                                               position_number: payment_system.adv_position_deposit }))
      Bybit::RateSnapshot.new(params.merge({ action: 'sell', payment_system:,
